...
export const sendMessage = async (chatRoomId: string, content: string, senderCompanyProfileId: string, receiverCompanyProfileId: string, pdfUrl?: string) => {
  try {
...
export const createOrGetChatRoom = async (
  ...
  try {
    ...
    const { data: existingRoom, error: fetchError } = await supabase
      .from('chat_rooms')
      .select('id')
      .or(`initiator_company_profile_id.eq.${initiatorCompanyProfileId},initiator_company_profile_id.eq.${recipientCompanyProfileId}`)
      .or(`recipient_company_profile_id.eq.${initiatorCompanyProfileId},recipient_company_profile_id.eq.${recipientCompanyProfileId}`)
      .single();
